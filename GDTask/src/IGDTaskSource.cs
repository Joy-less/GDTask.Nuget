...
/// </summary>
interface IGDTaskSource : IValueTaskSource
{
    // ValueTask compatibility
    ValueTaskSourceStatus IValueTaskSource.GetStatus(short token) => (ValueTaskSourceStatus)GetStatus(token);
    void IValueTaskSource.GetResult(short token) => GetResult(token);
    void IValueTaskSource.OnCompleted(Action<object> continuation, object state, short token, ValueTaskSourceOnCompletedFlags flags) => OnCompleted(continuation, state, token); // // ignore flags, always none.
...
interface IGDTaskSource<out T> : IGDTaskSource, IValueTaskSource<T>
{
    ...
    // ValueTask compatibility
    public new GDTaskStatus GetStatus(short token) => ((IGDTaskSource)this).GetStatus(token);
    public new void OnCompleted(Action<object> continuation, object state, short token) => ((IGDTaskSource)this).OnCompleted(continuation, state, token);
}
...
public static class GDTaskStatusExtensions
{
    ...
    public static bool IsCompleted(this GDTaskStatus status) => status != GDTaskStatus.Pending;

    /// <summary>status == Succeeded.</summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool IsCompletedSuccessfully(this GDTaskStatus status) => status == GDTaskStatus.Succeeded;

    /// <summary>status == Canceled.</summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool IsCanceled(this GDTaskStatus status) => status == GDTaskStatus.Canceled;

    /// <summary>status == Faulted.</summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool IsFaulted(this GDTaskStatus status) => status == GDTaskStatus.Faulted;
}