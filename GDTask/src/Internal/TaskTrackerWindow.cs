...
partial class TaskTrackerWindow : Window
{
    private static TaskTrackerWindow Instance;
    private static StringBuilder StringBuilder;

#if !NET7_0_OR_GREATER
    private static readonly Regex RemoveHrefLabel = new("<a href.+>(.+)</a>", RegexOptions.Compiled);
#else
    ...
    internal static void Launch()
    {
        ...
        foreach (var trackingData in TaskTracker.GetAllExistingTrackingData()) TryAddItem(trackingData);

        var window = ((SceneTree)Engine.GetMainLoop()).Root;
        window.CallDeferred(MethodName.AddChild, Instance);
        window.CallDeferred(MethodName.MoveChild, Instance, 0);
    }
    ...
    internal static void TryAddItem(TaskTracker.TrackingData trackingData)
    {
        ...
        lock (Instance)
        {
            var trackingDataMap = Instance._trackingDataMap;
            var treeItemMap = Instance._treeItemMap;
            var tree = Instance._tree;

            if (!trackingDataMap.TryGetValue(trackingData, out var treeItem))
            {
                treeItem = tree.CreateItem(Instance._rootTreeItem, trackingData.TrackingId);
    ...
    internal static void UpdateTreeItem(TaskTracker.TrackingData trackingData, TreeItem treeItem)
    {
        ...
        static string GetFirstLine(string str)
        {
            ...
            var constructed = StringBuilder.ToString();
            StringBuilder.Clear();
                ...
                marginContainer.AddThemeConstantOverride("margin_right", margin);
                {
                    ...
                    };
                    {
                        var toolbar = new HBoxContainer();
                        {
                            ...
                            TaskTracker.EnableTrackingObservable.Subscribe(new CheckButtonObserver(enableTrackingToggle));
                            TaskTracker.EnableStackTraceObservable.Subscribe(new CheckButtonObserver(enableStackTraceToggle));
    ...
    private TaskTrackerWindow(Tree tree, TextEdit textEdit)
    {
        _tree = tree;
        _textEdit = textEdit;
